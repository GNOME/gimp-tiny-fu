...
(define (tiny-fu-selection-round image
                                    drawable
                                   radius)
  ...
  (if (> select-width select-height)
      (set! cut-radius (truncate (+ 1 (* radius (/ select-height 2)))))
      (set! cut-radius (truncate (+ 1 (* radius (/ select-width 2))))))
...
(tiny-fu-register "tiny-fu-selection-round"
    _"<Image>/Tiny-Fu/Selection/_Round..."
